...
if ($checkbox.length) {
  $checkbox[0].addEventListener('change', function (event) {
    ...
    window.location = 'http://localhost:82/things/public/index.php?' + searchParams.toString();
  });
}
...
if ($taskCheckboxes.length) {
  ...
  $taskCheckboxes[0].addEventListener('change', function (event) {
    if (event.target.classList.contains('task__checkbox')) {
      ...
      console.log(task_id);
      var url = 'http://localhost:82/things/public/index.php?task_id=' + task_id + '&check=' + is_checked;
      window.location = url;
    }
  });
}
